...
module.exports = function (app) {
    ...
    function (req, res) {
      ...
      var query = connection.query(
        "SELECT c.customer_id, c.category, sum(c.amount) as budgeted_amount, " +
          ...
          "from customer_budget_details c " +
          "WHERE c.customer_id = ? and c.budgeted_month = ? and c.budgeted_year = ?  GROUP BY c.category",
        [userInput, thismonth, thisyear],
        function (err, result) {
          if (err) throw err;
          res.json(result);
  ...
  app.get("/api/monthlyexpenses/:userid/:month/:year", function (req, res) {
    ...
    var query = connection.query(
      "SELECT DATE_FORMAT(transaction_date, '%m-%d-%y') as t_date, sum(transaction_amount) as t_amt FROM customer_transactions " +
        ...
        "transaction_category <>  'Income' group BY transaction_date order by transaction_date",
      [userInput, inputmonth, inputyear],
      function (err, result) {
        if (err) throw err;
        res.json(result);
      }
  ...
  app.get("/api/monthlyexpensesbreakdown/:userid/:month/:year", function (
    ...
    var query = connection.query(
      "SELECT transaction_category, sum(transaction_amount) as t_amt FROM customer_transactions " +
        "WHERE customer_id = ? and budgeted_month = ? and budgeted_year = ? and  " +
        "transaction_category <>  'Income' group BY transaction_category order by transaction_category",
      [userInput, inputmonth, inputyear],
      function (err, result) {
        if (err) throw err;
        res.json(result);
  ...
  app.post("/api/createbudget", function (req, res) {
    ...
    connection.query(
      "INSERT INTO customer_budget_details(customer_id,category,label,amount,budgeted_month,budgeted_year)VALUES ?",
      [budget],
      function (error, results, fields) {
        if (error) {
          console.log(error);
